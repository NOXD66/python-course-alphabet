...
def task_4_update_customer(con):
    ...
    """
    with con.cursor() as cur:
        cur.execute("UPDATE customers SET customername = 'Johnny Depp' WHERE customerid IN (SELECT MIN(customerid) FROM customers);")
        con.commit()
    # pass
...
def task_8_count_customers_by_city(cur):
    ...
    """
    cur.execute("SELECT COUNT(customerid), city FROM customers GROUP BY city ORDER BY COUNT(customerid) DESC  ;")
    return cur.fetchall()
    # pass
...
def task_11_list_customers_starting_from_11th(cur):
    ...
    Returns: 11 records
    """
    cur.execute("SELECT * from customers where customerid NOT IN (SELECT customerid from customers LIMIT 11);")
    return cur.fetchall()
    # pass
